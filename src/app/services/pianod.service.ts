...
export class PianodService {
  ...
  public toggleStationInMix(station: string) {
    // if (this.mixList.value.length > 1 || !this.inMix(station)) {
...
// import {Subject} from 'rxjs/Subject';
//
// import {Message, Seed, Song, SongInfo, SongTime, User, UserInfo} from '../models';
//
// @Injectable()
// export class PianodService {
...
//     return new Promise((resolve, reject) => {
//       this.q.push(cmd, (res) => resolve(res));
//     });
//   }
//
//   public async login(userName, password) {
...
//     // observe response from socket messages
//     // const pianodMsgs$ = Observable.fromEvent(this.socket, 'message');
//     const pianodMsgs$ = Observable.fromEvent(this.socket, 'message').timeout(this.responseTimeout);
//
//     pianodMsgs$.filter((event: any) => Message.isValid(event.data))
//         .map((event: any) => new Message(event.data))
//         .takeUntil(response$)
